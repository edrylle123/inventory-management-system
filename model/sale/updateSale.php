<?php
	...
	if(isset($_POST['saleDetailsSaleID'])){
		...
		$fundCluster = htmlentities($_POST['saleDetailsFundCluster']);
		$orderID = htmlentities($_POST['saleDetailsOrderID']);
		...
		if(isset($saleDetailsItemNumber) && isset($saleDetailsSaleDate) && isset($saleDetailsQuantity) && isset($saleDetailsUnitPrice) && isset($saleDetailsCustomerID)){
			...
			// // Check if unit fund cluster
			// if($saleDetailsFundCluster == ''){ 
			// 	echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Please enter Fund Cluster.</div>';
			// 	exit();
			// }
			...
			if($originalSaleQuantityStatement->rowCount() > 0){
				...
				if($originalOrderItemNumber !== $saleDetailsItemNumber) {
					...
					$updateSaleDetailsSql = 'UPDATE sale SET itemNumber = :itemNumber, fundCluster = :fundCLuster, saleDate = :saleDate, itemName = :itemName, unitPrice = :unitPrice, discount = :discount, quantity = :quantity, customerName = :customerName, customerID = :customerID, fundCluster = :fundCluster  WHERE saleID = :saleID';
					$updateSaleDetailsStatement = $conn->prepare($updateSaleDetailsSql);
					$updateSaleDetailsStatement->execute(['itemNumber' => $saleDetailsItemNumber, 'saleDate' => $saleDetailsSaleDate, 'fundCluster' => $saleDetailsFundCluster, 'itemName' => $saleDetailsItemName, 'unitPrice' => $saleDetailsUnitPrice, 'discount' => $saleDetailsDiscount, 'quantity' => $saleDetailsQuantity, 'customerName' => $saleDetailsCustomerName, 'customerID' => $customerID, 'saleID' => $saleDetailsSaleID, 'fundCluster' =>$saleDetailsFundCluster]);
					...
					if($stockStatement->rowCount() > 0){
						...
						$updateSaleDetailsSql = 'UPDATE sale SET itemNumber = :itemNumber, saleDate = :saleDate, itemName = :itemName, unitPrice = :unitPrice, discount = :discount, quantity = :quantity, customerName = :customerName, customerID = :customerID, fundCluster = :fundCluster, oderID = :orderID  WHERE saleID = :saleID';
						$updateSaleDetailsStatement = $conn->prepare($updateSaleDetailsSql);
						$updateSaleDetailsStatement->execute(['itemNumber' => $saleDetailsItemNumber,'saleDate' => $saleDetailsSaleDate, 'itemName' => $saleDetailsItemName, 'unitPrice' => $saleDetailsUnitPrice, 'discount' => $saleDetailsDiscount, 'quantity' => $saleDetailsQuantity, 'customerName' => $saleDetailsCustomerName, 'customerID' => $customerID, 'saleID' => $saleDetailsSaleID, 'fundCluster' =>$saleDetailsFundCluster, 'orderID' => $saleDetailsOrderID]);
