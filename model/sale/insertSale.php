			...
			if($stockStatement->rowCount() > 0){
				...
				if($currentQuantityInItemsTable <= 0) {
					...
					if($customerStatement->rowCount() > 0){
						...
						$insertSaleSql = 'INSERT INTO sale(itemNumber, itemName, quantity, customerID, customerName, saleDate, unitPrice, orderID) VALUES (:itemNumber, :itemName, :quantity, :customerID, :customerName, :saleDate, :unitPrice, :orderID)';
						$insertSaleStatement = $conn->prepare($insertSaleSql);
						$insertSaleStatement->execute(['itemNumber' => $itemNumber, 'itemName' => $itemName, 'quantity' => $quantity, 'customerID' => $customerID, 'customerName' => $customerName, 'saleDate' => $saleDate, 'unitPrice' => $unitPrice, 'orderID' => $orderID]);
