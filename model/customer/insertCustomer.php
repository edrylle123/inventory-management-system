<?php
	...
	if(isset($_POST['customerDetailsCustomerFullName'])){
		...
		// $fundCluster = htmlentities($_POST['customerDetailsFundCluster']);
		...
		if(isset($fullName)) {
			...
			$sql = 'INSERT INTO customer(fullName, district, status, email) VALUES(:fullName, :district, :status, :email)';
			$stmt = $conn->prepare($sql); 
			$stmt->execute(['fullName' => $fullName, 'district' => $district, 'status' => $status, 'email' => $email ]);
